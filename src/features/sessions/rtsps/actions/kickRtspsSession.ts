...
export async function kickRtspsSession(id: string): Promise<{ ok: boolean, error?: string }> {
  const client = await getMediaMtxClient()
  if (!client) {
    return { ok: false, error: 'App config not initialised.' }
  }
  try {
    await client.api.v3.rtspsSessionsKick(id)
    revalidatePath('/sessions/rtsps')
    logger.info('Kicked RTSPS session', { id })
    return { ok: true }
  }
  catch (error) {
    logger.error('Failed to kick RTSPS session', { id, error })
