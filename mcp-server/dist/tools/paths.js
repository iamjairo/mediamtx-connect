...
export function registerPathTools(server, api, allowKick) {
    server.registerTool('mediamtx_paths_list', {
        title: 'List active paths',
        description: 'List runtime paths (with source, ready state, bytes counters).',
        annotations: { readOnlyHint: true },
        inputSchema: paginationSchema,
    }, async ({ page, itemsPerPage }) => callOrFail(api.v3.pathsList({ page, itemsPerPage })));
    server.registerTool('mediamtx_paths_get', {
    ...
    server.registerTool('mediamtx_config_paths_list', {
        title: 'List path configurations',
        description: 'List all configured paths (the static configuration, not runtime state).',
        annotations: { readOnlyHint: true },
        inputSchema: paginationSchema,
    }, async ({ page, itemsPerPage }) => callOrFail(api.v3.configPathsList({ page, itemsPerPage })));
    server.registerTool('mediamtx_config_paths_get', {
    ...
    server.registerTool('mediamtx_config_paths_add', {
        ...
        return callVoidOrFail(api.v3.configPathsAdd(name, config), `Path "${name}" created.`);
    });
    server.registerTool('mediamtx_config_paths_patch', {
        ...
        return callVoidOrFail(api.v3.configPathsPatch(name, config), `Path "${name}" patched.`);
    });
    server.registerTool('mediamtx_config_paths_replace', {
        ...
        return callVoidOrFail(api.v3.configPathsReplace(name, config), `Path "${name}" replaced.`);
    });
    server.registerTool('mediamtx_config_paths_delete', {
