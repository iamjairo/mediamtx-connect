...
import { useKickRtmpsConn, useRtmpsConns } from '../../hooks/queries'
import { formatBytes, formatRelative } from '../../lib/utils'
import type { RTMPConn } from '../../lib/mediamtx/generated'

export function RtmpsSessions() {
  const conns = useRtmpsConns({ itemsPerPage: 100 })
  const kick = useKickRtmpsConn()
      ...
      rowActions={row =>
        row.id
          ? (
              <KickButton
                id={row.id}
                label={`RTMPS conn ${row.id.slice(0, 8)}`}
